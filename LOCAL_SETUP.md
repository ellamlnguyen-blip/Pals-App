...
The owner profile editor is available at `/profile` after required onboarding. TASK-006 local verification uses `pnpm check`, `pnpm db:verify`, and `pnpm test:auth:web`. The latter now includes profile HTTP/action/failure tests and deterministic concurrent photo mutations; it starts/stops its own local web process and uses disposable accounts only. Stop another server on port 3000 before using the helper. UI verification should use a separate disposable local account after resets. No hosted profile migration or deployment is part of TASK-006.
