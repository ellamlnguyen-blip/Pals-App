...
## TASK-006 planning
Owner profile enrichment/photos has a bounded contract in `tasks/active/TASK-006-profile-enrichment.md` and Accepted ADR-0011 (explicit user acceptance 2026-09-22). Implementation is authorized on `agent/TASK-006-profile-enrichment`; a fresh implementation agent is active from verified main `fedf42c6a34fbf520f8823c675f99e4aa2ab42c3`. No implementation is yet integrated; local schema/UI/tests and fresh security review remain outstanding. It can proceed independently of TASK-005 using the existing identity/photo foundation. Owner-only access, TASK-003 hosted gaps and the ADR-0010 gate remain unchanged. No hosted operations are authorized. See `agents/handoffs/TASK-006-CONTRACT.md` for publication evidence.

TASK-006 validation milestone (unintegrated implementation): implementer reports `pnpm check` passed (13 unit tests, lint/typechecks and both builds) and `pnpm db:verify` passed two clean resets with 112 pgTAP assertions each plus schema lint. Initial independent static security review identified optional-whitespace normalization and ambiguous save-result copy; fixes are present, final re-review/execution evidence pending. HTTP failure/concurrency and rendered desktop/mobile checks remain in progress. No pushed implementation SHA yet; code stays outside main until accepted.
