...
## TASK-015B local Hangout notification verification

`local_notifications_hangouts.test.sql` covers actual-role source/table forgery denial, material public and private edits, normalized no-ops, failed/stale edits, join/leave/rejoin identity, removal and joining-state exclusions, joined-only fan-out, mute bypass for cancellation, and neutral projection after departure or cancellation. `notifications-hangouts-http.integration.mjs` uses real local Auth/PostgREST identities to verify owner pages, exact chat retry, no copied text, current membership and cancellation. `notifications-hangouts-concurrency.integration.mjs` observes PostgreSQL parent-row, recipient and gate lock waits for leave versus edit/send, removal versus edit, cancellation versus send, and both mute/gate commit orders. Run these serially with TASK-015A social and source regression suites against a validated loopback stack; each Node suite clears fixtures and restores gates.
