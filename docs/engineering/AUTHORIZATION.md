...
The authoritative edit, cancel, join and leave RPCs emit after their existing Hangout parent-row lock and committed transition checks. Chat emits only on a newly saved message after its Hangout and chat gate locks. A new event uses one server-owned identity shared by recipients; exact retries and normalized no-op edits create none. Fan-out reads current joined participants, excluding the actor; join/leave selects only the current host. It takes the notification gate row `FOR SHARE`, then recipient advisory locks in UUID order and rechecks each optional preference. A disabled gate or mute leaves the source mutation intact and does not backfill. Cancellation bypasses the optional update mute.
