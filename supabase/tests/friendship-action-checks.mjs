...
export async function friendshipActionChecks(owner, peer, a, b, png, sql) {
  ...
  try {
    ...
    assert.match(await action("createFriendRequest", [b.id, key1]), /No current relationship is visible/);
    ...
    assert.match(await action("createFriendRequest", [b.id, crypto.randomUUID()]), /No current relationship is visible/);
