...
## Disposable-local co-host authority (TASK-010A)

Migration `20260925000200_local_cohost_authority.sql` implements Accepted ADR-0012 under the existing default-off Hangout gate. `private.hangout_cohosts` stores an assignment tied to the participant key; a participant departure trigger clears it for ordinary leave, host removal, global-block teardown and reconciliation. An assignment grants authority only while the joined nonhost is live-ready on the same campus and the published Hangout remains undisabled. Readiness loss suspends authority, while readiness restoration can reactivate a still-joined assignment.

The caller-bound role RPCs are `promote_hangout_cohost(p_hangout_id,p_account_id,p_expected_revision)`, `demote_hangout_cohost(p_hangout_id,p_account_id,p_expected_revision)` and `step_down_hangout_cohost(p_hangout_id,p_expected_revision)`. Each returns the new revision. Host-only assignment discovery uses `list_hangout_cohosts(p_hangout_id,p_after_account_id,p_limit)`; current-ready roster labels use `list_hangout_roster_roles` with the same keyset pagination. Both cap pages at 24 and expose only account IDs plus permitted role labels. Neither provides profile data. The old two-argument removal RPC is dropped; `remove_hangout_participant(p_hangout_id,p_account_id,p_expected_revision)` now returns a revision. Co-host edits and joining control reuse the existing RPCs. Host cancellation remains exclusive. A cancelled Hangout permits only a revision increment for host removal or assigned nonhost leave, preserving its original `updated_at` for attendance eligibility.

All operations still require local READ COMMITTED, current gate/readiness/campus authority, and the shared social lock before the Hangout parent. No client table DML or gate grant was added. No hosted migration or persistent enablement is authorized.
