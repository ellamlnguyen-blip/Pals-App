...
## Implemented stages under Accepted ADR-0015
...
- [x] ADR-0015 accepted explicitly and published; narrower A/B contracts published before fresh dispatch; independent A review/integration before B.
- [x] One conversation is bound to each saved Hangout; a published Hangout's current live-ready same-campus joined host/participant can send/read bounded text and full retained history, including before a late join or during a permitted leave/rejoin gap. A discoverer, left/removed participant, cancelled Hangout participant, unready account, cross-campus caller or disabled gate cannot. Closed joining does not evict current participants.
