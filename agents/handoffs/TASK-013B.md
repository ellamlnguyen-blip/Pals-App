...
- Follow-up security review found that an early signout signal could mask and then reread while the old cookie was still valid. All in-app signout forms and the sign-in form now announce a pending transition before submission. Visible chat tabs stay masked while any transition token is pending; storage, focus, pageshow and polling cannot release it. A successful server action redirects with an intent-specific completion marker. The source tab broadcasts matching settlement only after that navigation, and each chat tab rechecks its own session and RPC authorization before showing text. Failed sign-in settles only after its action returns an error. Auth callback navigation broadcasts a separate revalidation signal. The completion marker is removed from the URL by the root notifier.
...
Plan and tests: `docs/ux/TASK-013B-INTERACTION-PLAN.md`, `supabase/tests/chat-ui-http.integration.mjs`, `tests/auth-transition.test.mjs`.

## Tests / Verification

- `pnpm check`: pass after the auth follow-up and completion-marker refinement (format, lint, types, 22 Node tests, web/admin builds). `git diff --check`: pass.
...
- The focused real local HTTP test was rerun and passed after the follow-up UI changes. The temporary test and visual accounts, Hangouts, messages and ledgers were removed. Final local query returned Hangout gate `false`, chat gate `false`, zero chat messages and zero `chat-auth-%` Auth users. The web server, Supabase services and Lima VM were stopped; ignored `.env.local` and temporary fixture files were removed. No hosted environment was touched.
...
- Browser automation in this task-agent thread was unavailable. The coordinator's rendered check covered layout, successful multiline send, cross-tab signout, different-account denial, Back reauthorization and final textarea focus. Hidden/pagehide, lost-response retry, changed draft after uncertainty and stale in-flight read after denial were not independently exercised in a browser. The focused HTTP suite verifies database and route outcomes, and the focused Node test verifies transition-token decisions; neither replaces browser lifecycle coverage. The exact completion-marker refinement was not browser-retested after the local stack was stopped.
