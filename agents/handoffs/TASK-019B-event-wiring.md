...
- `pnpm check` passed after correction: formatting, lint, workspace typechecks, 47 tests and web/admin production builds. Focused evidence tests cover first-key success versus retry, unknown/stale/denied, accepted transition, and the absence of a friend-request runtime capture call site.
