...
- P1 notification fix regression: new pgTAP **10/10** covers visible co-host edit, readiness suspension/restoration, departure neutralization, and immutable host edit. Existing Hangout notifications **48/48** and co-host authority **63/63** still pass. Real Auth/PostgREST co-host suite passed **1/1** with co-host actor privacy checks across readiness loss/restoration and departure. After a clean reset the complete database suite passed **19 files / 1,010 assertions**, no `not ok` or SQL errors; a second clean reset applied the same migration successfully. Final schema lint again found no errors. After the final notification migration was complete, a separate true prior-schema upgrade reset locally through `20260925000100`, restored the final migration file, applied it with `supabase migration up --local`, and verified `20260925000200` immediately above `20260925000100` in migration history. The complete pgTAP suite passed again on that upgraded schema: **19 files / 1,010 assertions**. A repeat lint command on this upgraded local stack exited 137 without output; the earlier schema lint pass used the same final migration SQL, which did not change in this test-only follow-up.
- Expanded observed-lock suite passed **1/1** with 12 observed wait cases: leave→promote, demote→edit, cancellation→co-host removal, cancellation→queued host removal, host removal→promote, demote→joining, demote→removal, moderation disable→edit, actor readiness loss→edit, target readiness loss→promote, source-gate off→promote, and global block→promote. It verifies no unauthorized role/write, target state, and block provenance for the affected pair. Because moderator disable evidence is immutable, fixtures were removed through a clean local reset.
- After the coordinator restored locked dependencies in this isolated worktree, the full **`pnpm check` passed (exit 0)**: Prettier formatting, ESLint, all workspace TypeScript checks, root tests (**46 pass, 1 sandbox loopback skip**), and both Next production builds. Earlier direct-tool runs had timed out while the dependency installation was incomplete; those timeouts do not describe the final workspace result. The backend agent did not modify dependencies in this follow-up.
- After the timestamp-capture regex was made explicit for CR/LF exclusion, the same **12-case observed-lock suite passed 1/1** again via direct `node` execution. The queued host removal assertion compares the exact cancellation timestamp captured by the leader with the post-removal database value.
