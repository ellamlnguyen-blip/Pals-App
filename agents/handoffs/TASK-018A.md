...
- All 16 `supabase/tests/database/*.test.sql` files passed directly through `psql -v ON_ERROR_STOP=1` in the disposable Supabase Postgres container, including 57 new attendance pgTAP assertions. Existing Hangout, account, block, safety, moderation, chat and notification SQL regressions passed.
- `python3 supabase/tests/attendance-concurrency.integration.py` passed 18 observed two-session outcomes: both commit orders for attendance/source gate changes, account sanction, caller-bound leave, caller-bound cancellation, host title edit, moderator disable, host block, plus answer versus rejected post-opening host schedule edit. The test uses actual authenticated roles and the existing source/moderation RPCs. Block, leave and after-start cancellation correctly retain self-answer eligibility.
...
- The two-session suite used direct privileged gate/account/block updates to model revocations. Leave, cancellation, host edits and disable were exercised through their real caller-bound RPCs. It does not constitute a real external moderator or student session.
