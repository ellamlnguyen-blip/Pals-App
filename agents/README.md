...
`origin/main` is the canonical integration baseline and shared reference for every agent. Start each task branch from its latest commit. The task agent commits and pushes its finished work and handoff on its task branch, then verifies and records the remote SHA. After reviewing the result, the orchestrator integrates accepted work into `main`, pushes it, and records the resulting SHA. A task is complete only after both refs are verified. Use the local Git credential manager; credentials never belong in chat or the repository. Do not force-push or push task branches directly into `main`.
